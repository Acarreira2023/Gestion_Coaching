...
import { toDate, esMismoDia, esMismoMes } from "../utils/dateUtils";
import {
  itemsIngreso,
  categoriasIngreso,
  itemsEgreso,
...
export function useReportData(params = {}) {
  ...
  return useMemo(() => {
    ...
    // Flatten de itemsIngreso y itemsEgreso
    const allItemsIngreso = Object.values(itemsIngreso).flat().map((it) => it.value);
    const allItemsEgreso = Object.values(itemsEgreso).flat().map((it) => it.value);

    // Categorías ya son arrays
    const allCategoriasIngreso = categoriasIngreso.map((c) => c.value);
    const allCategoriasEgreso = categoriasEgreso.map((c) => c.value);

    // Usar en mkPie
    const ingresosByItem = mkPie(filtI, allItemsIngreso, "item");
    const ingresosByCategoria = mkPie(filtI, allCategoriasIngreso, "categoria");
    const egresosByItem = mkPie(filtE, allItemsEgreso, "item");
    const egresosByCategoria = mkPie(filtE, allCategoriasEgreso, "categoria");
